...
router.put('/changepassword', validateToken, async (req, res) => {
  ...
  if (!match) {
    res.status(404).json({ message: 'Old password incorrect, please try again.' });
    return;
  }
  const newHashedPw = await bcrypt.hash(newPw, 10);
  await Users.update({ password: newHashedPw }, { where: { id: userId } });
  res.status(202).json({ message: 'Password was changed successfully :)' });
});
