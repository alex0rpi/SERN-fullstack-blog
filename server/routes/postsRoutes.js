...
router.put('/update', validateToken, async (req, res) => {
  const { field, newInfo, id } = req.body;
  if (field === 'title') {
    await Posts.update({ title: newInfo }, { where: { id: id } });
  }
  if (field === 'body') {
    await Posts.update({ postText: newInfo }, { where: { id: id } });
  }
  res.status(200).json({ updatedPost: { field, newInfo } });
});
